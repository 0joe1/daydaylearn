...
def encrypt(text,seed):
    ...
    keys = KeyGenerator(Tmatrix(text2matrix(seed)))

    AddRoundKey(matrix,keys[:4])
    for i in range(1,10):
        SubBytes(matrix)
        ShiftRows(matrix)
        MixColumns(matrix)
        AddRoundKey(matrix,keys[i*4:(i+1)*4])
    SubBytes(matrix)
    ShiftRows(matrix)
    AddRoundKey(matrix,keys[40:44])
...
def decrypt(cipher,keys):
    ...
    AddRoundKey(matrix,keys[40:44])
    Inv_ShiftRows(matrix)
    Inv_SubBytes(matrix)

    for i in range(9,0,-1):
        AddRoundKey(matrix,keys[i*4:(i+1)*4])
        Inv_MixColumns(matrix,i)
        Inv_ShiftRows(matrix)
        Inv_SubBytes(matrix)
    AddRoundKey(matrix,keys[:4])
...
print(plaintext)
t = encrypt(plaintext,seed)
#print(t)
print(decrypt(t,keys))

#print(plaintext)
#t = text2matrix(plaintext)
#print(matrix2text(t))
